...
function DetailsScreen({ route, navigation }) {
 ...
 let arr = [fajr, dhuhr, asr, maghrib, isha];
let s= _retrieveData = async () => {
  try {
    const value = await AsyncStorage.getItem(JSON.stringify(item1));
    if (value !== null) {
  ...
  return (
    <>
      ...
      <View style={{ flexDirection: "row", flex: 1, justifyContent: "space-between", margin: 20 }}>
        <Text style={{ fontWeight: 'bold', fontSize: 17 }}>Fajr (Sunrise Prayer)</Text>
        <BouncyCheckbox
        value={fajr}
      ...
      <Text>
        ...
        <Button title="Add Record" onPress={_storeData = async () => {
          try {
            await AsyncStorage.setItem(JSON.stringify(item1),
            JSON.stringify(arr)
            );
          } catch (error) {
            // Error saving data
